<resource schema="ibis">
  ...
  <meta name="creationDate">2016-07-20</meta>
  ...
  <meta name="_longdoc" format="rst"> <![CDATA[
    ...
    We concatenated the 3x52 catalogues in the single table. On each subfield,
    the three per-band catalogues are made of the same (J selected) sources. We
    concatenated them prefixing the columns with `j_`, `h_`, and `k_`; with
    these exceptions:

    - The position (`alpha_j2000` and `delta_j2000`) comes from the J band, thus
      there is only two columns not prefixed.

    - Some other columns are the same for each band and must come from the
      J processing. We kept the `j_` prefix but we did not include the columns
      from the H and Ks catalogues: `j_x_image`, `j_y_image`, `j_imaflags_iso`,
      `j_nimaflags_iso`, `j_threshold`, `j_x2_image`, `j_y2_image`,
      `j_xy_image`, `j_x2_world`, `j_y2_world`, `j_xy_world`, `j_cxx_image`,
      `j_cyy_image`, `j_cxy_image`, `j_cxx_world`, `j_cyy_world`, `j_cxy_world`,
      `j_errcxx_image`, `j_errcyy_image`, `j_errcxy_image`, `j_errcxx_world`,
      `j_errcyy_world`, `j_errcxy_world`, `j_a_world`, `j_b_world`,
      `j_erra_world`, `j_errb_world`, `j_theta_world`, `j_theta_j2000`,
      `j_errtheta_world`, `j_errtheta_j2000`, `j_elongation`, `j_ellipticity`.

    As we concatenated *vertically* the catalogues, there are duplicated sources
    in the overlapping areas between field. We added a `subfield` column
    containing the name of the subfield associated to each source. As we kept
    the `number` column (a sequential identifier in each subfield) it's easy to
    go back to the original catalogue.
    ...
    ======== ============================================
    20160720 First ingestion of the catalogue
    ======== ============================================
